-- TODO: minimap icons, see "MiniMapEntity" and "maprevealable"
...
local function MorphSnowChester(inst)
    ...
    inst.components.preserver:SetPerishRateMultiplier(TUNING.PERISH_COLD_FROZEN_MULT)  -- unlimited freshness, no need for persihable stop code
...
local function CanMorph(inst)
    if inst.ChesterState ~= "NORMAL" or not TheWorld.state.isnight then
        return false, false
    end
