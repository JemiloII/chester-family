...
local function SpawnCavester(inst)
	local pt = inst:GetPosition()
	local spawn_pt = GetSpawnPoint(pt)
	if spawn_pt ~= nil then
		local cavester = SpawnPrefab("cavester")
		if cavester ~= nil then
			cavester.Physics:Teleport(spawn_pt:Get())
			cavester:FacePoint(pt:Get())
			return cavester
		end
	end
end
...
local function RebindCavester(inst, cavester)
	if not TheWorld.ismastersim then
		return inst
	end
	cavester = cavester or TheSim:FindFirstEntityWithTag("cavester")
	if cavester ~= nil then
		...
		if cavester.components.follower.leader ~= inst then
			cavester.components.follower:SetLeader(inst)
		end
		return true
	end
end

local function RespawnCavester(inst)
	StopRespawn(inst)
	RebindCavester(inst, TheSim:FindFirstEntityWithTag("cavester") or SpawnCavester(inst))
end

StartRespawn = function(inst, time)
	StopRespawn(inst)
	time = time or 0
	inst.respawntask = inst:DoTaskInTime(time, RespawnCavester)
	inst.respawntime = GetTime() + time
	inst.AnimState:PlayAnimation("dead_idle_loop", true)
	CloseEye(inst)
end

local function FixCavester(inst)
	inst.fixtask = nil
	if not RebindCavester(inst) then
		inst.AnimState:PlayAnimation("dead_idle_loop", true)
		CloseEye(inst)
...
local function OnPutInInventory(inst)
	if inst.fixtask == nil then
		inst.fixtask = inst:DoTaskInTime(1, FixCavester)
	end
end
...
local function fn()
    ...
    inst:AddTag("glowbone")
    inst:AddTag("irreplaceable")
    inst:AddTag("nonpotatable")

	inst.AnimState:SetBank("glowbone")
	inst.AnimState:SetBuild("glowbone")
	inst.AnimState:PlayAnimation("idle_loop", true)
	...
	inst.openEye = "glowbonealive" -- to glowbone when you make the inventoryimage
	inst.closedEye = "glowbonedead" -- to glowbone_closed when you make the inventoryimage
...
return Prefab("common/inventory/glowbone", fn, assets)
